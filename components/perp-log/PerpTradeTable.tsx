...
interface PerpTradeTableProps {
  trades: PerpTrade[];
  onEdit: (trade: PerpTrade) => void;
  onDelete: (id: string) => void;
  onFilteredDataChange: (data: PerpTrade[]) => void;
}

const formatDate = (timestamp: number) => new Date(timestamp).toLocaleDateString('en-US', { year: '2-digit', month: 'short', day: 'numeric' });
...
export const PerpTradeTable = ({ trades, onEdit, onDelete, onFilteredDataChange }: PerpTradeTableProps) => {
      ...
      return (
          <Card className="border-dashed border-gray-600">
              <CardContent className="flex flex-col items-center justify-center h-full min-h-[300px] text-center p-6">
                  ...
                  <h3 className="text-xl font-bold text-gray-300">Your Perpetuals Log is Empty</h3>
                        ...
                        <TableBody>
                            {filteredData.map((trade) => (
                                <TableRow key={trade.id}>
                                    <TableCell className="whitespace-nowrap">{formatDate(trade.entryDate)}</TableCell>
                                    <TableCell className="font-medium text-white">{trade.symbol}</TableCell>
                                    <TableCell><Badge variant={trade.side === 'Long' ? 'success' : 'danger'}>{trade.side}</Badge></TableCell>
                                    <TableCell><Badge variant={trade.status === 'Open' ? 'default' : 'warning'}>{trade.status}</Badge></TableCell>
                                    <TableCell className={`text-right font-mono ${!trade.pnl ? 'text-gray-400' : trade.pnl > 0 ? 'text-green-400' : 'text-red-400'}`}>
                                        {trade.status === 'Closed' ? formatCurrency(trade.pnl) : '—'}
                                    </TableCell>
                                    <TableCell className="text-right font-mono text-gray-300">{formatCurrency(trade.entryPrice)}</TableCell>
                                    <TableCell className="text-right font-mono text-cyan-400">{formatCurrency(trade.margin)}</TableCell>
                                    <TableCell className="text-right font-mono text-gray-300">{trade.leverage}x</TableCell>
